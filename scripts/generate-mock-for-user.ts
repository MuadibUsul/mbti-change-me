...
const mockCount = Number(process.argv[3] ?? 20);

if (!targetEmail) {
  console.error("Usage: npx tsx scripts/generate-mock-for-user.ts <email> [count]");
  process.exit(1);
}
...
function buildPlannedScores(count: number, rng: seedrandom.PRNG, start?: DimensionScores): DimensionScores[] {
...
async function main() {
  const rng = seedrandom(`mock:${targetEmail}:v3`);
  ...
  if (!user) {
    ...
    console.log(`Created user ${targetEmail} with password Admin123456`);
  }
  ...
  const historyRecords = existingCompleted.map((s) => ({
    mbtiType: s.mbtiType,
    behaviorStats: s.behaviorStats,
    dimensionScores: s.dimensionScores.map((d) => ({ dimension: d.dimension, normalizedScore: d.normalizedScore })),
  }));

  const historyPoints = existingCompleted.map((s) => ({
    createdAt: s.createdAt.toISOString(),
    scores: toScoreMap(s.dimensionScores.map((d) => ({ dimension: d.dimension, normalizedScore: d.normalizedScore }))),
  ...
  const plannedScores = buildPlannedScores(mockCount, rng, latestScores);

  const now = Date.now();
  const dayMs = 24 * 60 * 60 * 1000;
  const startOffsetDays = mockCount * 2;
  let inserted = 0;
  let persistedStyleDNA = Boolean(styleDNA);

  for (let i = 0; i < mockCount; i += 1) {
    const sessionStart = new Date(now - (startOffsetDays - i * 2) * dayMs);
    const questionCount = 36;
    const currentPersona = buildPersonaModel(historyRecords);

    const generatedQuestions = generateQuestions(questionCount, `${targetEmail}:session:${i}:${sessionStart.getTime()}`, {
    ...
    const completionSeconds = Math.round(360 + rng() * 600);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!persistedStyleDNA && styleDNA) {
        await tx.user.update({ where: { id: user.id }, data: { styleDNA } });
  ...
  const completed = await prisma.testSession.count({ where: { userId: user.id, status: "COMPLETED" } });
  console.log(JSON.stringify({ email: targetEmail, inserted, completed }, null, 2));
}
